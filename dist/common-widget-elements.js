!function(DEFAULT_CDN_PREFIX, STRING_WEB_COMPONENTS_REQUESTED, WINDOW, DOCUMENT, init) {
    /**
     ...
     */
    if ("customElements" in WINDOW) init(); else {
        ...
        if (WINDOW.AWAITING_WEB_COMPONENTS_POLYFILL) {
            WINDOW.AWAITING_WEB_COMPONENTS_POLYFILL.then(init);
            return;
        }
        var _WEB_COMPONENTS_REQUESTED = WINDOW.AWAITING_WEB_COMPONENTS_POLYFILL = getCallbackQueue();
        _WEB_COMPONENTS_REQUESTED.then(init);
        var WEB_COMPONENTS_POLYFILL = WINDOW.WEB_COMPONENTS_POLYFILL || "//cdnjs.cloudflare.com/ajax/libs/webcomponentsjs/2.0.2/webcomponents-bundle.js";
        var ES6_CORE_POLYFILL = WINDOW.ES6_CORE_POLYFILL || "//cdnjs.cloudflare.com/ajax/libs/core-js/2.5.3/core.min.js";
        "Reflect" in WINDOW || WINDOW.SKIP_ES6_CORE_POLYFILL ? loadScript(WEB_COMPONENTS_POLYFILL).then(function() {
            _WEB_COMPONENTS_REQUESTED.isDone = true;
            _WEB_COMPONENTS_REQUESTED.exec();
        }) : loadScript(ES6_CORE_POLYFILL).then(function() {
    ...
    function getCallbackQueue() {
        ...
        callbacks.then = function(callback) {
            callbacks.isDone ? callback() : callbacks.push(callback);
            return callbacks;
        };
        return callbacks;
    }
...
}(0, 0, // Global wait queue var name
